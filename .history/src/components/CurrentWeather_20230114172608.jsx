...
function CurrentWeather(){
    ...
    const months = ["January","February","March","April","May","June","July","August","September","October","November","December"]
     ...
     useEffect(()=>{
      if(weatherData.length !== 0){
        function setCurrectTime(givenTime){
          if(givenTime == 0){
  ...
  return(
    ...
    {hourlyData[0].map((element, index)=>(<HourlyForecast key={index} tempProp={element.temp_c} iconProp={element.condition.icon} timeProp={setCurrectTime(new Date(`${element.time}`).getHours())}/>))}
    </div>
    </div>}
    </>
  )
