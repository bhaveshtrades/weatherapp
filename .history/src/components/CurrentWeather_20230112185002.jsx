...
function CurrentWeather(){
  ...
  return(
    ...
    {hourlyData.length !== 0 && hourlyData[0].map((element, index)=>(<HourlyForecast key={index} tempProp={element.temp_c} iconProp={element.condition.icon} timeProp={new Date(`${element.time}`).getTime()}></HourlyForecast>))}
    </div>
    </>
  )
}
