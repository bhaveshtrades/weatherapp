...
function CurrentWeather(){
    ...
    const[currectTime, setCorrectTime] = useState();
    ...
    const months = ["January","February","March","April","May","June","July","August","September","October","November","December"]
     ...
     useEffect(function dailyForecasting(element, index){
      if(hourlyData.length !== 0){
        if(new Date(`${element.time}`).getHours() == 0){
          setCorrectTime(12);
        }else if(new Date(`${element.time}`).getHours() > 12){
          setCorrectTime(new Date(`${element.time}`).getHours() - 12);
          setTimeUnit('PM');
        }
      }
      return <HourlyForecast key={index} tempProp={element.temp_c} iconProp={element.condition.icon} timeProp={currectTime+timeUnit}/>
     }, [weatherData])
  ...
  return(
    ...
    {/* {hourlyData[0].map((element, index)=>(<HourlyForecast key={index} tempProp={element.temp_c} iconProp={element.condition.icon} timeProp={new Date(`${element.time}`).getHours()}/>
    ))} */}
