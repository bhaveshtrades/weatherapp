...
function CurrentWeather(){
  ...
  return(
    ...
    <div className='mt-4 pl-52'>
    <div>
    <h3>Today's Weather</h3>
    </div>
    <div className='flex'></div>
    {hourlyData.length !== 0 && hourlyData.map((element, index)=><HourlyForecast key={index} tempProp={hourlyData[0].element.temp_c} iconProp={hourlyData[0].element.condition.icon} timeProp={new Date(`${hourlyData[0].element.time}`).getTime()}></HourlyForecast>)}
    </div>
    </>
  )
}
