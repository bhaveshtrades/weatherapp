...
function CurrentWeather(){
    ...
    const months = ["January","February","March","April","May","June","July","August","September","October","November","December"]
     ...
     function getWeather(e){
      ...
      .then((response)=>{
        ...
        setHourlyData([response.forecast.forecastday[0].hour[0][0]]);
        }else{
        setWeatherData([]);
        }
      });
  ...
  return(
    ...
    {hourlyData.length !== 0 && hourlyData.map((element, index)=>(<HourlyForecast key={index} tempProp={element.temp_c} iconProp={element.condition.icon} timeProp={new Date(`${element.time}`).getTime()}></HourlyForecast>))}
    </div> */}
    </>
  )
}
