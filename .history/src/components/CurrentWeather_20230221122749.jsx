...
function CurrentWeather(){
  ...
  return(
    ...
    <h2 className='text-base pl-8 md:pl-16 md:text-xl'>{weatherData[0].location.name}, {weatherData[0].location.country}</h2>
    <h4 className='text-sm pl-8 md:pl-16 md:text-base'>{new Date(`${weatherData[0].current.last_updated}`).getDate()} {months[new Date(`${weatherData[0].current.last_updated}`).getMonth()]}, {weekDays[new Date(`${weatherData[0].current.last_updated}`).getDay()]}</h4>
    <div> 
    <div className='flex justify-center gap-x-10 w-max h-max ml-16 px-2 py-1 currentForecastItem mt-4 md:ml-28'>
