...
function Navbar({resultProp, cityProp}){
  ...
  return (
    ...
    <form role="search" onSubmit={resultProp} className='flex gap-x-2'>
        ...
        <input type='text' className='text-center w-44 h-8' placeholder='Enter City Name' onChange={cityProp} id='cityName'/>
        <button type="submit" className='btn btn-dark w-16 h-8 text-center' onClick={resultProp}>Search</button>
    </form>
    </div>
    </>
  )
