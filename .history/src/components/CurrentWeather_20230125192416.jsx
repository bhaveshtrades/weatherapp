...
function CurrentWeather(){
  ...
  return(
    ...
    {weatherData.length !== 0 && <div className='mt-16 w-4/5 ml-8 h-72 currentWeatherDiv'>
    <h2 className='text-base pl-8 pt-2 bg-red-500'>{weatherData[0].location.name}, {weatherData[0].location.country}</h2>
    <h4 className='text-sm pl-8'>{new Date(`${weatherData[0].current.last_updated}`).getDate()} {months[new Date(`${weatherData[0].current.last_updated}`).getMonth()]}, {weekDays[new Date(`${weatherData[0].current.last_updated}`).getDay()]}</h4>
    <div> 
    <div className='flex gap-x-10 mt-4 pl-8'>
