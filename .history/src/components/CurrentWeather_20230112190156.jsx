...
function CurrentWeather(){
  ...
  return(
    ...
    <div className='mt-4 ml-52 w-3/4 h-1/6 bg-green-400'>
    <div>
    <h3>Today's Weather</h3>
    </div>
    <div className='flex gap-x-12 pl-4 pt-2 overflow-y-scroll'>
    {hourlyData.length !== 0 && hourlyData[0].map((element, index)=>(<HourlyForecast key={index} tempProp={element.temp_c} iconProp={element.condition.icon} timeProp={new Date(`${element.time}`).getHours()}/>))}
    </div>
    </div>
    </>
  )
