...
function CurrentWeather(){
  ...
  return(
    ...
    {weatherData.length !== 0 && <div className='mt-16 w-5/6 ml-8 pl-4 h-72 currentWeatherDiv'>
    <h2 className='text-base'>{weatherData[0].location.name}, {weatherData[0].location.country}</h2>
    <h4 className='text-sm'>{new Date(`${weatherData[0].current.last_updated}`).getDate()} {months[new Date(`${weatherData[0].current.last_updated}`).getMonth()]}, {weekDays[new Date(`${weatherData[0].current.last_updated}`).getDay()]}</h4>
    <div className='flex gap-x-72'> 
    <div className='flex gap-x-10 mt-6'>
    <div>
    <img className='bg-red-500 ml-4' src={`${weatherData[0].current.condition.icon}`} alt='This is a weather Icon'/>
    </div>
    <div>
    <h2 className='text-lg'>{weatherData[0].current.temp_c}<span>&#176;</span></h2>
    ...
    {weatherData.length !== 0 && <div className='w-5/6 ml-8 h-64 forecastingDiv'>
