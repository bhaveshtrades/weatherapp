...
function CurrentWeather(){
    ...
    const months = ["January","February","March","April","May","June","July","August","September","October","November","December"]
     ...
     function getWeather(e){
      ...
      if(hourlyData.length !== 0){
        function setCorrectTime(givenTime){
          if(givenTime == 0){
            return setTimeChange(12);
          }else if(givenTime > 12){
            return setTimeChange(24 - givenTime);
          }else{
            return setTimeChange(givenTime);
          }
        }
      }
  ...
  return(
    ...
    {hourlyData[0].map((element, index)=>(<HourlyForecast key={index} tempProp={element.temp_c} iconProp={element.condition.icon} timeProp={setCorrectTime(new Date(`${element.time}`).getHours())}/>))}
    </div>
    </div>}
    </>
  )
