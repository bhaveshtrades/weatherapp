...
function Navbar({resultProp, cityProp}){
  ...
  return (
    ...
    <form role="search" onSubmit={resultProp} className='flex gap-x-4'>
        <label htmlFor='cityName' className='text-slate-50 font-bold text-sm mt-2  w-52 h-8'>Enter City: </label>
        <input type='text' className='text-center w-52 h-8' placeholder='Enter City Name' onChange={cityProp} id='cityName'/>
        <button type="submit" className='btn btn-dark w-52 h-8' onClick={resultProp}>Search</button>
    </form>
    </div>
    </>
  )
