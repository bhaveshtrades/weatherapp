...
function App() {
    ...
    const currentAPI = `http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${city}`;
     ...
     const getWeather = (e)=>{
      ...
      .then((response)=>{
        ...
        // setCurrentIcon(response.current.conditon.code);
  ...
  return (
    ...
    {weatherData.length !==0 && <CurrentWeather countryProp={country} locationProp={location} dayProp={weekDays[currentDay]} dateProp={currentDate} monthProp={months[currentMonth]}  currentTempProp={currentTemp} currentStatusProp={currentStatus}/>}
    </>
  );
}
